...
STAR --runMode genomeGenerate --genomeDir ${rootdir}/STAR_Index/ --genomeFastaFiles ${rootdir}/vector_sequence/pSIN-TRE-TFs-3-HA-puroR_BC_final.fa --genomeSAindexNbases 5 --runThreadN 12 --runDirPerm All_RWX
...
for ((i = 0; i < ${#exp_array[@]}; i++)); do
	...
	STAR --runMode alignReads --twopassMode Basic --outSAMmapqUnique 60 --runThreadN 48 --genomeDir  ${rootdir}/STAR_Index --outFilterMultimapNmax 1 --readFilesCommand zcat --outSAMtype BAM Unsorted --outFileNamePrefix ${rootdir}/bam/ --readFilesIn ${rootdir}/fastq/C3H10_10X_${exp}_enriched_S1_L001_R2_001.fastq.gz
	...
	java -jar TFseqTools-1.0.jar Counter --r1 ${rootdir}/fastq/C3H10_10X_${exp}_enriched_S1_L001_R1_001.fastq.gz --r2 ${rootdir}/bam/C3H10_10X_${exp}_enriched.bam --tf ${rootdir}/metadata/C3H10_10X_Metadata_${exp}.txt -p BU --UMI ${umi_length} --BC 16 >${rootdir}/results/C3H10_10X_${exp}_enriched.tf_counting.log
