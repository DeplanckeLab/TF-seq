...
seu <- Seurat_int[, (
  ((Seurat_int@meta.data$batch %in% c("exp9", "exp10", "exp11")) & (Seurat_int@meta.data$TF %in% c("D0"))) |
...
# add adiposcore
# we add it here so that we use the same "scale" for all combinations
...
seu <- AddModuleScore(seu, features = list(adipo = adipo.markers), name = "adiposcore")
