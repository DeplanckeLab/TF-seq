...
seu <- seu %>% FindNeighbors(verbose = FALSE)
...
seu$seurat_clusters <- seu$integrated_snn_res.1.1

# set color scheme
Cols_res_1 <- c("#FFFFCC","#CC9966","#FFCC99","#FF9933","#FF9999","#CC6666",
                "#FF99FF","#FFCCCC","#99FF99","#CCCC00","#339900","#CC99CC",
                "#66CCCC","#3399FF","#CCFF00","#660066","#FFFF00","#CC00FF","#003300")
names(Cols_res_1) <- as.character(0:18)

# plot the atals on UMAP
p_umap <- DimPlot(seu, group.by = "seurat_clusters", order = T, label = T, cols = Cols_res_1)+
...
### extract clustering metadata at "integrated_snn_res.1.1"

```{r Fig_S4b, fig.width=5, fig.height=6}

df <- data.frame(row.names = rownames(seu@meta.data),
                 TF = seu$TF,
                 Cluster = seu$integrated_snn_res.0.8)
...
df_percent$percentage_TF <- round(df_percent$percentage_TF, digits = 2)
...
```{r}
DimPlot(seu, cells.highlight = colnames(seu)[seu$TF == "D0_confluent"])
DimPlot(seu, cells.highlight = colnames(seu)[seu$TF == "D0"])

```
```{r}
table(seu$integrated_snn_res.1[seu$TF == "Etv1"])
```
```{r}
for (i in seq(0.1,1.5,0.1)){table(subset(seu, TF == "Etv1")[[paste0("integrated_snn_res.",i)]]) %>% print()}
```
...
```{r}
  data.seurat_integrated_phase <- JackStraw(data.seurat_integrated_phase, assay = "integrated", reduction = "pca", dims = 200) # 1h
  data.seurat_integrated_phase <- ScoreJackStraw(data.seurat_integrated_phase, reduction = "pca", dims = 1:200)
  JackStrawPlot(data.seurat_integrated_phase, dims = 1:100)
```
