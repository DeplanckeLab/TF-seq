...
# be careful about the step of ens-id to gene symbol conversion, the order might be messed up
rownames(heatmap_data) <- data.annot[rownames(heatmap_data), "gene_short_name"]
...
my_sample_col <- cds@colData[,"Dose", drop = F]
my_sample_col$Pseudotime <- Order.pseudotime[rownames(my_sample_col)]
my_sample_col$Cluster <- colData(cds)$meta.new 
...
heatmap <- pheatmap::pheatmap(as.matrix(heatmap_data),
                              ...
                              cluster_cols = F, cluster_rows = F, scale = "row", clustering_method = "ward.D2",
                              annotation_legend = T
)

ggsave(heatmap, file = "figures/Fig_S4.1d.pdf", width = 8, height = 10)
```
